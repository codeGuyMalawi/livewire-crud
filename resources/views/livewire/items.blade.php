<div class="p-6 sm:px-20 bg-white border-b border-gray-200">
    ...
    @if (session()->has('message'))
        <div style="background-color: #3584B3" class="flex items-center bg-blue-500 text-white text-sm font-bold px-4 py-3 relative" role="alert" x-data="{show:true}" x-show="show">
    ...
    <div class="mt-8 text-2xl flex justify-between">
        ...
        <div class="mr-1">
            <x-jet-button wire:click="confirmItemAdd" wire:loading.attr="disabled">
                {{ __('Add New Item') }}
            </x-jet-button>
        </div>
    ...
    <div class="mt-6">
        <div class="flex justify-between">
            <div class="">
                <input type="search" wire:model.debounce.500ms='searchItem' placeholder="search"
                    class="block p-2 pl-4 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            </div>
            <div class="mr-2">
                ...
                @foreach ($items as $item)
                    <tr>
                        ...
                        <td class="border px-4 py-2">
                            <x-jet-button class="bg-blue-500 hover:bg-blue-400" wire:click="confirmItemEdit({{ $item->id }})"
    ...
    <x-jet-dialog-modal wire:model="confirmingItemAdd">
        ...
        <x-slot name="footer">
            ...
            <x-jet-danger-button class="ml-3" wire:click="saveItem()" wire:loading.attr="disabled">
                {{ isset($this->item->id) ? 'Update' : 'Save' }}
            </x-jet-danger-button>
        </x-slot>
    </x-jet-dialog-modal>
